...
public class M11ReferenceAssetLoaderTest {
    ...
    private static final int[][] ISO = {{0,10000},{10000,20000},{20000,40000},{40000,200000}};
    ...
    @Test
    public void loadsBandOneAndMaterializesImplicitAxes() throws Exception {
        byte[] assetBytes = fixture(false);
        M11ReferenceAssetLoader.Asset asset = M11ReferenceAssetLoader.load(
                new ByteArrayInputStream(assetBytes), 15000);

        assertEquals(1, asset.metadata.selectedCc1BandIndex);
        assertEquals(15000, asset.metadata.requestedIso);
        assertEquals(M11ReferenceAssetLoader.EXPECTED_FIRMWARE_SHA256, asset.metadata.firmwareSha256);
        assertFalse(asset.metadata.thirdSroActiveInPixelChain);
        assertArrayEquals(SRO, asset.metadata.sroWords);
        assertEquals(4, asset.metadata.cc1Bands.length);
        assertEquals(10000L, asset.metadata.cc1Bands[1].lowerInclusive);
        assertEquals(20000L, asset.metadata.cc1Bands[1].upperExclusive);

        assertEquals(910.0 / 512.0, asset.tables.cc1[0], 0.0);
        assertEquals(-290.0 / 512.0, asset.tables.cc1[1], 0.0);
        assertEquals(1024, asset.tables.toneX.length);
        assertEquals(4096, asset.tables.gammaX.length);
        assertEquals(1.0, asset.tables.toneX[1023], 0.0);
        assertEquals(1.0, asset.tables.gammaX[4095], 0.0);
        assertEquals(asset.tables.toneX[512], asset.tables.toneCurve(-3)[512], 0.0);
        assertTrue(asset.tables.gammaY[4095] <= 1.0);
    }
    ...
    public void rejectsTamperedPayload() throws Exception {
        ...
        try {
            ...
            fail("tampered payload should fail SHA-256 trailer validation");
    ...
    @Test
    public void rejectsAttemptToActivateUnresolvedThirdSro() throws Exception {
        byte[] bytes = fixture(true);
        try {
            M11ReferenceAssetLoader.load(new ByteArrayInputStream(bytes), 100);
    ...
    @Test
    public void rejectsIsoOutsideExtractedBandEvidence() throws Exception {
        try {
            M11ReferenceAssetLoader.load(new ByteArrayInputStream(fixture(false)), 250000);
            fail("out-of-range ISO should require explicit evidence rather than nearest-band guessing");
        } catch (IOException expected) {
    ...
    private static byte[] fixture(boolean activateThirdSro) throws Exception {
        ...
        for (int i = 0; i < 4; i++) {
            out.writeInt(ISO[i][0]);
            out.writeInt(ISO[i][1]);
            writeI16(out, CC1[i]);
        }
        ...
        // Synthetic serialization fixture only.  Production data can be emitted
        // solely by the Python packer after exact canonical source hash gating.
        for (int state = 0; state < 7; state++) {
            for (int i = 0; i < 1024; i++) out.writeShort(4096);
        }
        for (int i = 0; i < 4096; i++) out.writeShort((int)Math.round(i * 1023.0 / 4095.0));

        int[][] modes = {{0,-1,-1,511,100},{1,0,0,588,115},{2,1,1,665,130}};
        for (int[] mode : modes) {
            out.writeByte(mode[0]);
            out.writeByte(mode[1]);
            out.writeByte(mode[2]);
            out.writeByte(0);
            out.writeShort(mode[3]);
            out.writeShort(mode[4]);
        }
        for (int word : SRO) out.writeInt(word);
        out.writeByte(activateThirdSro ? 1 : 0);
        out.writeByte(0);
        out.writeByte(0);
        out.writeByte(0);
        out.flush();
