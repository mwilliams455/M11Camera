...
std::vector<double> copyArray(JNIEnv *env, jdoubleArray source, jsize expected, const char *name) {
    ...
    if (count != expected) throw std::invalid_argument(std::string(name) + " length mismatch");
...
std::array<double, 3> rawWbFromNeutral(const std::vector<double>& neutral) {
    if (neutral.size() != 3) throw std::invalid_argument("AsShotNeutral must contain three values");
    for (double value : neutral) {
        if (!(value > 0.0) || !std::isfinite(value)) {
            throw std::invalid_argument("AsShotNeutral must be finite and positive");
        }
    }
    const double max_neutral = std::max(neutral[0], std::max(neutral[1], neutral[2]));
    return {max_neutral / neutral[0], max_neutral / neutral[1], max_neutral / neutral[2]};
}

m11::render::Mat3 residualAfterRawWb(const m11::render::Mat3& original,
                                     const std::array<double, 3>& wb) {
    m11::render::Mat3 residual = original;
    for (int row = 0; row < 3; ++row) {
        for (int col = 0; col < 3; ++col) {
            residual[static_cast<std::size_t>(row * 3 + col)] /= wb[static_cast<std::size_t>(col)];
        }
    }
    return residual;
}

double cancellationError(const m11::render::Mat3& original,
                         const m11::render::Mat3& residual,
                         const std::array<double, 3>& wb) {
    double max_error = 0.0;
    for (int row = 0; row < 3; ++row) {
        for (int col = 0; col < 3; ++col) {
            const std::size_t i = static_cast<std::size_t>(row * 3 + col);
            max_error = std::max(max_error, std::abs(residual[i] * wb[static_cast<std::size_t>(col)] - original[i]));
        }
    }
    return max_error;
}

jobject runB2rWbPlacementRender(JNIEnv *env,
                               int source_fd,
    ...
    const auto neutral = copyArray(env, as_shot_neutral_array, 3, "AsShotNeutral");
    const auto raw_wb = rawWbFromNeutral(neutral);
    const auto residual_camera_to_m11 = residualAfterRawWb(camera_to_m11, raw_wb);
    const double cancel_error = cancellationError(camera_to_m11, residual_camera_to_m11, raw_wb);
    if (!(cancel_error <= 1e-12)) {
        throw std::runtime_error("B2RWBPLACE1A: matrix cancellation guard failed");
    }
    ...
    if (!stream.valid()) throw std::runtime_error("B2RWBPLACE1A: invalid source fd");

    LibRaw raw;
    int rc = raw.open_datastream(&stream);
    if (rc != LIBRAW_SUCCESS) {
        throw std::runtime_error("B2RWBPLACE1A: open_datastream failed: " + std::string(LibRaw::strerror(rc)));
    }

    const std::string runtime = LibRaw::version();
    if (runtime.rfind("0.22.1", 0) != 0) {
        throw std::runtime_error("B2RWBPLACE1A: LibRaw version gate failed: " + runtime);
    }
    ...
    if (raw.get_decoder_info(&decoder) != LIBRAW_SUCCESS) {
        throw std::runtime_error("B2RWBPLACE1A: decoder identity unavailable");
    }
    if (!equalsField(id.make, kMake) || !equalsField(id.model, kModel) ||
        ...
        throw std::runtime_error("B2RWBPLACE1A: metadata/decoder gate failed; refusing render");
    }
    ...
    if (rc != LIBRAW_SUCCESS) {
        throw std::runtime_error("B2RWBPLACE1A: unpack failed: " + std::string(LibRaw::strerror(rc)));
    }

    // Start from the frozen REALRAW1C/rawpy oracle, then change only the
    // channel multipliers.  highlight=Clip in the frozen oracle makes
    // LibRaw scale_colors() normalize by the minimum pre_mul.  Because
    // max(neutral)/neutral always has minimum 1, this preserves the baseline
    // global scale while moving the source WB diagonal ahead of AHD.
    m11raw::applyRawpy0271OracleParams(raw.imgdata.params, false);
    raw.imgdata.params.user_mul[0] = static_cast<float>(raw_wb[0]);
    raw.imgdata.params.user_mul[1] = static_cast<float>(raw_wb[1]);
    raw.imgdata.params.user_mul[2] = static_cast<float>(raw_wb[2]);
    raw.imgdata.params.user_mul[3] = static_cast<float>(raw_wb[1]);

    const auto ahd_start = std::chrono::steady_clock::now();
    rc = raw.dcraw_process();
    if (rc != LIBRAW_SUCCESS) {
        throw std::runtime_error("B2RWBPLACE1A: AHD process failed: " + std::string(LibRaw::strerror(rc)));
    }
    ...
    if (image == nullptr || mem_error != LIBRAW_SUCCESS) {
        throw std::runtime_error("B2RWBPLACE1A: memory image failed");
    }
    struct ImageGuard {
        LibRaw &raw;
    ...
    if (image->type != LIBRAW_IMAGE_BITMAP || image->bits != 16 || image->colors != 3) {
        throw std::runtime_error("B2RWBPLACE1A: unexpected AHD output type");
    }
    ...
    if (image->data_size != expected_bytes) {
        throw std::runtime_error("B2RWBPLACE1A: unexpected AHD byte count");
    }

#if __BYTE_ORDER__ != __ORDER_LITTLE_ENDIAN__
#error "B2RWBPLACE1A is defined for little-endian uint16 LibRaw output"
#endif

    jobject bitmap = createArgb8888Bitmap(env, static_cast<int>(image->width), static_cast<int>(image->height));
    ...
    if (AndroidBitmap_getInfo(env, bitmap, &bitmap_info) != ANDROID_BITMAP_RESULT_SUCCESS ||
        ...
        throw std::runtime_error("B2RWBPLACE1A: unexpected Android bitmap layout");
    }
    void *pixels = nullptr;
    if (AndroidBitmap_lockPixels(env, bitmap, &pixels) != ANDROID_BITMAP_RESULT_SUCCESS || pixels == nullptr) {
        throw std::runtime_error("B2RWBPLACE1A: AndroidBitmap_lockPixels failed");
    }
    ...
    try {
        ...
        for (unsigned y = 0; y < image->height; ++y) {
            ...
            for (unsigned x = 0; x < image->width; ++x) {
                ...
                const auto m11_rgb = m11::render::multiply(residual_camera_to_m11, camera_rgb);
    ...
    if (AndroidBitmap_unlockPixels(env, bitmap) != ANDROID_BITMAP_RESULT_SUCCESS) {
        throw std::runtime_error("B2RWBPLACE1A: AndroidBitmap_unlockPixels failed");
    }

    std::ostringstream report;
    report << std::setprecision(17);
    report << "schema=m11camera.render1h.b2rwbplace1a.v1\n";
    report << "candidate=B2RWBPLACE1A\n";
    ...
    report << "demosaic=AHD\n";
    report << "sourceWbPlacement=pre_AHD_LibRaw_scale_colors\n";
    report << "sourceWbCompensation=post_AHD_cameraToM11_times_inverse_D\n";
    report << "asShotNeutralR=" << neutral[0] << "\n";
    report << "asShotNeutralG=" << neutral[1] << "\n";
    report << "asShotNeutralB=" << neutral[2] << "\n";
    report << "rawWbR=" << raw_wb[0] << "\n";
    report << "rawWbG=" << raw_wb[1] << "\n";
    report << "rawWbB=" << raw_wb[2] << "\n";
    report << "rawWbG2=" << raw_wb[1] << "\n";
    report << "linearCancellationMaxAbsError=" << cancel_error << "\n";
    report << "mode=Standard\n";
    report << "sourceNormalization=rgb16/65535.0\n";
    report << "additionalDownstreamSourceWB=false\n";
    report << "leicaRendererTablesChanged=false\n";
    ...
    if (result == nullptr) throw std::runtime_error("B2RWBPLACE1A result allocation failed");
    jstring diagnostics = env->NewStringUTF(report.str().c_str());
    if (diagnostics == nullptr) throw std::runtime_error("B2RWBPLACE1A diagnostics allocation failed");
    env->SetObjectArrayElement(result, 0, bitmap);
    env->SetObjectArrayElement(result, 1, diagnostics);
    if (env->ExceptionCheck()) throw std::runtime_error("B2RWBPLACE1A result population failed");
    return result;
}
...
Java_com_m11_diagnostic_M11RenderBridge_nativeRenderRealXiaomiStandardB2rWbPlacementFd(
        ...
        jdoubleArray gamma_y_array) {
    try {
        return static_cast<jobjectArray>(runB2rWbPlacementRender(
                env, static_cast<int>(source_fd), camera_to_m11_array,
                as_shot_neutral_array, cc0_array, cc1_array, tone_x_array,
                tone_flat_array, gamma_x_array, gamma_y_array));
    } catch (const std::exception &e) {
        throwJavaState(env, e.what());
        return nullptr;
    } catch (...) {
        throwJavaState(env, "B2RWBPLACE1A: unknown native failure");
        return nullptr;
    }
}
