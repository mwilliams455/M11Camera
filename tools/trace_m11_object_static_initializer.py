...
Unknown instructions invalidate only the destination register they visibly
write when practical; no execution of firmware is attempted.
"""
...
def report(d):
    ...
    for i in ins:
        ...
        if m in ('str','strb','strh'):
            ...
            continue
        # common vector-zero stores are intentionally omitted; they are not
        # needed to resolve major type/subtype/external ID/payload pointer.
    ...
    for idx in sorted(records):
        ...
        lines.append('')
    lines += ['## Interpretation boundary','', 'This proves constant initialization only for fields written by the straight-line generator. Missing vector/dynamic fields are not assumed to be zero. The generated table may touch fewer than 0x210 records even though accessors reserve that capacity.','']
    return '\n'.join(lines)
def main():
