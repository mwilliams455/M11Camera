#!/usr/bin/env python3
"""Trace Leica M11-P transport engine 0x0194a7b0.

The selector packet path is now:
  BB06 selector -> 0x015765b4 -> 0x0194a5a4 -> 0x0194ac40 -> 0x0194a7b0
with the last three preserving (channel/context-id, command, packet, length).
This pass maps 0x0194a7b0 and its direct callees/callers to determine whether the
next boundary is queue/IPC/device transport. No subsystem name is assumed.
"""
...
def nearest_prologue(d,c,r=0x1600):
    ...
    return best
def next_prologue(d,s,max_len=0x3000):
    md=Cs(CS_ARCH_ARM,CS_MODE_ARM|CS_MODE_LITTLE_ENDIAN)
    for o in range(s+4,min(len(d)-4,s+max_len),4):
        i=next(md.disasm(d[o:o+4],o),None)
        if is_prologue(i): return o
    return min(len(d),s+max_len)
def callers(d,t): return [o for o in range(SCAN_START,min(SCAN_END,len(d)-4)&~3,4) if bl_target(o,u32(d,o))==t]
...
def report(d):
    ...
    lines=['# M11-P R2A transport engine trace','',f'- SHA-256: `{h}`',f'- target: `0x{TARGET:08x}`',f'- body bound: `0x{start:08x}–0x{end:08x}` (`0x{end-start:x}` bytes)',f'- direct A32 callers: `{len(cs)}`',f'- direct target-body callees: `{len(direct)}`','', '## Direct callees','']
    for t,n in direct.most_common(80): lines.append(f'- `0x{t:08x}`: `{n}` call(s)')
    lines += ['', '## Target body','', '```text']; lines.extend(disasm(d,start,end,700)); lines += ['```','', '## Caller windows','']
    for c in cs[:120]:
    ...
    lines += ['## Interpretation boundary','', 'Queue, IPC, mailbox or device semantics are not assigned unless target-body register/diagnostic/callee evidence supports them.','']
    return '\n'.join(lines)
def main():
