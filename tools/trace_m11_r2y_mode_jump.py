...
first table word.  This probe decodes all 17 entries and follows each case
until its first terminal branch/return, exposing mode-specific hardware/API
calls that a linear CFG walker misses.
"""
...
def main():
    ...
    # Verify the jump instruction itself before trusting table semantics.
    j = one(c, d, JUMP)
    ...
    for idx, t in enumerate(entries):
        ...
        lines.append(f'- selector `{idx:2d}`: word @ `0x{TABLE+4*idx:08X}` = `0x{t:08X}`' + ('' if valid else ' **OUTSIDE EXPECTED CODE REGION**'))
    ...
    lines += ['', 'RENDER1H remains frozen. This probe only resolves the mode jump that the prior CFG trace intentionally did not model.', '']
