...
def main():
    ...
    def fstart(addr,win=0x20000):
        for p in range(addr&~3,max(START,(addr&~3)-win),-4):
            if ispush(u32(d,p)):return p
        return max(START,addr-0x1000)
    callmap=defaultdict(list)
    for q in range(0,len(code)-3,4):
        t=bt(START+q,struct.unpack_from('<I',code,q)[0])
        if t is not None:callmap[t].append(START+q)
    def callers(t):return callmap.get(t,[])
    md=Cs(CS_ARCH_ARM,CS_MODE_ARM|CS_MODE_LITTLE_ENDIAN);md.detail=True;md.skipdata=True
    def dis(lo,hi):return list(md.disasm(d[lo:hi],lo))
    L=['# M11-P ColorSpec frame consumer trace v2','',f'- SHA256 `{h}`','']
    ...
    for q in range(0,len(code)-4,4):
        ...
        for r in range(q+4,min(q+36,len(code)-3),4):
            ...
            if mt and mt[0]==rd:
                if ((mt[1]<<16)|lo)==GLOBAL:refs.append((START+q,START+r,rd))
                break
    L+=['','## Global ColorSpec object direct MOVW/MOVT references',f'- base `0x{GLOBAL:08X}` refs `{[(hex(x),hex(y),r) for x,y,r in refs]}`']
    gfuncs=sorted(set(fstart(x) for x,_,_ in refs))
    for fs in gfuncs:
        rr=[x for x,_,_ in refs if fstart(x)==fs]
        L+=['',f'### global-ref function `0x{fs:08X}` callers `{[hex(x) for x in callers(fs)]}`','```asm']
        L += [fmt(i) for i in dis(max(fs,min(rr)-0x40),max(rr)+0x100)]+['```']
    ...
    by=defaultdict(list)
    for addr,f,text in hits:by[fstart(addr)].append((addr,f,text))
    L+=['','## Frame-field immediate/displacement census',f'- hit functions: `{len(by)}`','']
    for fs,rows in sorted(by.items()):
        L += [f'### function `0x{fs:08X}` callers `{[hex(x) for x in callers(fs)]}`']
        for addr,f,text in rows:L.append(f'- `{FIELDS[f]}`: `{text}`')
        shown=set()
        for addr,f,text in rows:
            bucket=addr&~0x7f
            if bucket in shown:continue
            shown.add(bucket);L+=['```asm']+[fmt(i) for i in dis(max(fs,addr-0x48),addr+0x70)]+['```']
        L.append('')
    ...
    for t,name,span in ((0x016CDE50,'upstream CM/AAA containing function',0x1300),(0x016F2108,'ColorSpec top-level orchestration',0x650),(0x016F2C84,'ColorSpec entry wrapper',0x100)):
        L += ['',f'## {name} `0x{t:08X}`',f'- callers `{[hex(x) for x in callers(t)]}`','```asm']
        L += [fmt(i) for i in dis(t,t+span)]+['```']
