...
GET_FRAME=0x01721858
CM_FINISH=0x016EB09C
R2Y_CTRL=0x01B1CDA4
R2Y_BUILDER=0x0172C19C
R2Y_MCC_WRITER=0x01B2D324
...
def main():
    ...
    callmap=defaultdict(list); callees=defaultdict(list)
    for i in ins:
        t=bl_target(i.address,u32(d,i.address))
        if t is not None:
            callmap[t].append(i.address); callees[fstart(d,i.address)].append((i.address,t))
    ...
    getter_calls=callmap.get(GET_FRAME,[])
    getter_funcs=sorted(set(fstart(d,x) for x in getter_calls))
    ...
    for off,name in FIELDS.items():
        ...
        for ha in hits[off]:
            fs=fstart(d,ha); n=idx.get(ha)
            has_get=fs in getter_funcs
            near_r2y=[]
            for ca,t in callees.get(fs,[]):
                if t in (R2Y_CTRL,R2Y_BUILDER,R2Y_MCC_WRITER,GET_FRAME,CM_FINISH): near_r2y.append((ca,t))
            L += [f'### hit `0x{ha:08X}` func `0x{fs:08X}` GET_FRAME={has_get} special_calls=`{[(hex(x),hex(t)) for x,t in near_r2y]}`','```asm']
            if n is not None:
    ...
    for fs in getter_funcs:
        end=next_push(d,fs); seq=[x for x in ins if fs<=x.address<end]
        touched=[]
        for x in seq:
            vals=immediates(x)
            for off,name in FIELDS.items():
        ...
        specials=[(ca,t) for ca,t in callees.get(fs,[]) if t in (R2Y_CTRL,R2Y_BUILDER,R2Y_MCC_WRITER,GET_FRAME,CM_FINISH)]
    ...
    for t in (R2Y_CTRL,R2Y_BUILDER,R2Y_MCC_WRITER):
        L.append(f'- `0x{t:08X}` callers `{[hex(x) for x in callmap.get(t,[])]}`')
    # dump caller contexts for any R2Y entry for manual bridge checking
    for t in (R2Y_CTRL,R2Y_BUILDER,R2Y_MCC_WRITER):
        for ca in callmap.get(t,[]):
            n=idx.get(ca); fs=fstart(d,ca)
