...
EXPECTED_HASHES = {
    ...
    "category42_saturation.json": "5869a69073a698f506b86c068261d0640b90e93da20627d9d7a7926b043c46cb",
...
EXPECTED_CC1_RANGES = ((0, 10000), (10000, 20000), (20000, 40000), (40000, 200000))
EXPECTED_YCC = ((77, 150, 29), (-43, -85, 128), (128, -107, -21))
EXPECTED_THIRD_SRO = ((212, -165, -71), (-73, 676, 85), (-27, 174, 285))
MODE_RECORDS = (
    # id, name, contrast state, saturation state, Cat42 field, chroma percent
...
def hash_gate(canonical_dir: Path) -> tuple[dict[str, str], dict[str, Any]]:
    ...
    firmware_sha = source.get("sha256")
    if firmware_sha != EXPECTED_FIRMWARE_SHA256:
        raise ValueError(f"unexpected firmware SHA-256: {firmware_sha}")
...
def parse_canonical(canonical_dir: Path) -> dict[str, Any]:
    ...
    for index, band in enumerate(bands_raw):
        ...
        interval = (int(iso[0]), int(iso[1]))
        if interval != EXPECTED_CC1_RANGES[index]:
            raise ValueError(f"Category13 band {index}: unexpected ISO range {interval}")
        bands.append({"iso_lower": interval[0], "iso_upper_exclusive": interval[1], "matrix_q9": flatten3(band.get("matrix_q9"), f"CC1 band {index}")})
...
def build_binary(parsed: dict[str, Any]) -> bytes:
    ...
    # Fixed-point denominators and cardinalities.
    ...
    # Full SRO core is provenance/audit data only.  The final byte is an explicit
    # consumer-placement flag and MUST stay zero until firmware evidence closes it.
...
def write_manifest(path: Path, binary_path: Path, binary: bytes, parsed: dict[str, Any]) -> None:
    ...
    obj = {
        ...
        "source_firmware": {
            ...
            "canonical_manifest_sha256": sha256_file(path.parent.parent / "canonical" / "manifest.json") if (path.parent.parent / "canonical" / "manifest.json").is_file() else None,
        },
...
def main() -> None:
    ...
    write_manifest(args.out_manifest, args.out_bin, binary, parsed)
    print(json.dumps({
