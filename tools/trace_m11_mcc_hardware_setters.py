...
The scanner finds the exact ARM MOVW/MOVT base-table load from raw instruction
words, bounds functions from ARM push/pop encodings, and uses Capstone only on
each unique candidate function. No renderer arithmetic is inferred here.
"""
...
def arm_mov_imm(w: int, kind: str) -> tuple[int, int] | None:
    tag = w & 0x0FF00000
...
def find_base_hits(data: bytes) -> list[int]:
    ...
    for p in range(LO, stop - 4, 4):
        dec = arm_mov_imm(word(data, p), "movw")
        if dec is None or dec[1] != 0x1224:
            continue
        rd = dec[0]
        for q in range(p + 4, min(p + 32, stop), 4):
            dec2 = arm_mov_imm(word(data, q), "movt")
            if dec2 == (rd, 0x4320):
                hits.append(p)
                break
    return hits
...
def is_push_lr(w: int) -> bool:
    # AL STMDB sp!, reglist (PUSH alias), with LR in reglist.
    return (w & 0xFFFF0000) == 0xE92D0000 and bool(w & 0x4000)


def is_return(w: int) -> bool:
    # AL LDMIA sp!, reglist (POP alias) with PC, or BX LR.
...
def find_function_window(data: bytes, hit: int) -> tuple[int, int]:
    fn = hit
    for p in range(hit, max(LO, hit - 0x8000), -4):
        if is_push_lr(word(data, p)):
            fn = p
            break
    end = min(HI, fn + 0x10000)
    for p in range(max(hit + 4, fn + 4), min(HI, fn + 0x10000), 4):
        if is_return(word(data, p)):
            end = p + 4
            break
    return fn, end
...
def parse_mem_disp(op_str: str) -> list[int]:
    return [int(m.group(1), 0) for m in re.finditer(r"\[[^\]]+?,\s*#(0x[0-9a-f]+|[0-9]+)\]", op_str)]


def nearest_landmark(x: int) -> tuple[str, int] | None:
    best = min(LANDMARKS, key=lambda k: abs(x-k))
    delta = x - best
    return (LANDMARKS[best], delta) if abs(delta) <= 0x7C else None
...
def main() -> None:
    ...
    for h in hits:
        ...
        windows.setdefault((fn, end), []).append(h)

    rows = []
    decoder = md()
    for (fn, end), refs in sorted(windows.items()):
        ins = list(decoder.disasm(data[fn:end], fn))
        adds, disps = [], []
        for x in ins:
        ...
        bank_score = 100 if 0x1000 in addvals else 0
        if 0x4000 in addvals: bank_score += 5
        if 0x2000 in addvals: bank_score -= 15
        if 0x5000 in addvals: bank_score -= 5

        exact = [(addr, d, LANDMARKS[d], mn, ops) for addr, d, mn, ops in disps if d in LANDMARKS]
        lm = []
        for addr, d, mn, ops in disps:
            near = nearest_landmark(d)
            if near:
                lm.append((addr, d, near[0], near[1], mn, ops))
        score = bank_score + len({x[2] for x in exact}) * 8 + min(len(exact), 20)
        if score > 0:
            rows.append((score, fn, end, refs, adds, exact, lm, ins))

    rows.sort(reverse=True)
    lines = [
        "# M11-P F_R2Y.MCC hardware-setter scan", "",
        f"- unpacked SHA-256: `{digest}`",
        f"- raw per-pipe base references: `{len(hits)}`",
        f"- unique base-referencing functions: `{len(windows)}`",
        f"- ranked candidates: `{len(rows)}`",
        "- address closure: common R2Y `+0x4000` == public `0xC000`; MCC `0x9000` => target `perPipeBase +0x1000`", "",
        "## Ranked candidates", "",
        "| score | function | base refs | bank adds | exact MCC landmarks |",
        "| ---: | --- | ---: | --- | --- |",
    ]
    for score, fn, end, refs, adds, exact, lm, ins in rows[:50]:
        addvals = sorted({v for _,_,v,_ in adds if v >= 0x800})
    ...
    for rank, row in enumerate(rows[:20], 1):
        score, fn, end, refs, adds, exact, lm, ins = row
        lines += ["", f"## Candidate {rank}: `0x{fn:08x}..0x{end:08x}` score {score}", "",
                  f"base refs: `{[hex(x) for x in refs]}`",
                  f"bank/add instructions: `{[(hex(a), m, hex(v), ops) for a,m,v,ops in adds if v >= 0x800]}`",
                  f"exact landmarks: `{[(hex(a), hex(d), name) for a,d,name,_,_ in exact]}`", "",
                  "### MCC-near accesses", ""]
        for a0, d, name, delta, mn, ops in lm[:320]:
            lines.append(f"- `0x{a0:08x}` `{mn} {ops}` -> {name} `{delta:+#x}`")
        lines += ["", "### Address-anchor excerpt", "", "```asm"]
        for x in ins:
            if any(k in x.op_str for k in ("#0x1224", "#0x4320", "#0x1000", "#0x2000", "#0x4000", "#0x5000")):
                lines.append(f"0x{x.address:08x}: {x.mnemonic} {x.op_str}")
        lines += ["```"]

    lines += ["", "## Interpretation boundary", "",
              "Close a Leica MCC programmer only when the per-pipe +0x1000 bank selection and concrete public MCC register footprint agree. Category adjacency is not evidence.", ""]
